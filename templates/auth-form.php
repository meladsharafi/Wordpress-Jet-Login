...
<div class="container__auth-form m-0 <?php echo  $args['view-modal-form'] ?  'hidden' : '' ?> w-72 md:w-80 flex-col mx-auto fixed top-5 md:top-[20%] right-1/2 translate-x-1/2 z-[9999]">
  <form id="auth-form" class="relative p-5 md:p-7 flex flex-col gap-3 bg-white shadow-lg rounded-xl overflow-hidden duration-1000" action="" method="post" data-url="<?php echo admin_url('admin-ajax.php') ?>">
    ...
    <?php if ($args['view-modal-form']): ?>
      ...
      <div class="mx-auto text-center max-h-">
        ...
        <a href="<?php echo get_site_url() ?>">
          <img class="max-h-28" src="<?php echo esc_url(wp_get_attachment_image_src(get_theme_mod('custom_logo'), 'full')[0]) ?>" alt="">
        </a>
        ...
        <input class="phone-number__auth-form py-2 bg-gray-100 rounded-md text-center" type="tel" name="phone-number" maxlength="11" placeholder="شماره موبایل را وارد کنید" autofocus>
        
        <div class="container-otp-code__auth-form hidden items-center gap-3">
          <p class="whitespace-nowrap">کد تایید:</p>
          <input class="otp-code__auth-form w-full py-2 bg-gray-100 rounded-md text-center" type="tel" name="otp-code" maxlength="4">
        </div>
        
        <div class="container-btn__auth-form relative">
          <button id="btn-submit__auth-form" class="btn-main">ارسال کد</button>
    ...
    <div id="footer__auth-form" class="flex flex-col gap-3">
      <span id="countdown__auth-form"></span>
      <p class="status__auth-form hidden mb-0" data-auth-message="status"></p>
    </div>
    <?php wp_nonce_field('mel_sms_auth_ajax_nonce', 'msa-nonce') ?>
  </form>
